...
def prompt_for_date(prompt: str) -> datetime:
    ...
    while not valid:
        try:
            ...
            minute: int = int(input('Enter minutes --> '))
            second: int = int(input('Enter seconds --> '))
...
def select_general(cls):
    ...
    index_menu = Menu('which search option', 'Which search option do you want to search:', choices)
    while True:
        ...
        if cls.objects(**filters).count() == 1:
            ...
            print('Sorry, no objects found that match that criteria.  Try again.')
